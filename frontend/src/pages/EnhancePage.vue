...
import { uploadImages } from "../features/artifacts/api";
...
const resultUrl = computed(() => {
  const id = task.value?.job.output_artifact_id;
  return id ? `/api/v1/artifacts/${encodeURIComponent(id)}/content` : null;
});
...
<template>
  ...
  <form class="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]" @submit.prevent="submit">
    <section class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
      ...
      <label class="mt-8 flex min-h-56 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 px-6 text-center transition hover:border-indigo-400 hover:bg-indigo-50/40">
        ...
        <input class="sr-only" type="file" accept="image/*" multiple @change="selectFiles">
      </label>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-6">
      ...
      <div class="mt-6 space-y-5">
        <div class="grid grid-cols-2 gap-2 rounded-lg bg-slate-100 p-1 text-sm">
          <button type="button" :class="backend === 'traditional' ? 'bg-white text-slate-950 shadow-xs' : 'text-slate-500'" class="rounded-md px-3 py-2 font-medium" @click="backend = 'traditional'">Traditional</button>
          <button type="button" :class="backend === 'deep' ? 'bg-white text-slate-950 shadow-xs' : 'text-slate-500'" class="rounded-md px-3 py-2 font-medium" @click="backend = 'deep'">Deep model</button>
        </div>
        ...
        <button type="submit" :disabled="loading || loadingCatalog" class="w-full rounded-lg bg-slate-950 px-4 py-3 text-sm font-medium text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50">{{ loading ? 'Uploading and submitting…' : 'Submit enhancement task' }}</button>
      </div>
    </section>
  </form>
  ...
  <section v-if="task" class="mt-6 rounded-2xl border border-slate-200 bg-white p-6">
    ...
    <p v-if="task.message" class="mt-4 text-sm text-slate-600">{{ task.message }}</p>
    <a v-if="resultUrl" :href="resultUrl" target="_blank" rel="noreferrer" class="mt-5 inline-flex rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-500">Open result artifact</a>
    <RouterLink to="/tasks" class="mt-5 ml-3 inline-flex text-sm font-medium text-slate-500 hover:text-slate-950">View all tasks</RouterLink>
  </section>
</template>
