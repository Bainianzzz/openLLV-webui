...
<template>
  ...
  >
    <template #actions><Badge variant="secondary">Dataset worker</Badge></template>
  </PageHeader>
  ...
  <div class="grid gap-6 xl:grid-cols-[minmax(320px,0.75fr)_minmax(0,1.25fr)]">
    <Card>
      <CardHeader>
        ...
        <CardDescription>Only dataset keys advertised by the server catalog can be submitted.</CardDescription>
      </CardHeader>
      <CardContent>
        <form class="space-y-5" @submit.prevent="submit">
          <div class="space-y-2">
            <Label for="dataset-key">Configured dataset</Label>
            <Select v-model="form.datasetKey" :disabled="loadingCatalog || !datasetKeys.length">
              <SelectTrigger id="dataset-key" class="w-full">
                <SelectValue :placeholder="loadingCatalog ? 'Loading datasets…' : 'Select a dataset'" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem v-for="datasetKey in datasetKeys" :key="datasetKey" :value="datasetKey">
                  {{ datasetKey }}
                </SelectItem>
              </SelectContent>
            </Select>
            <p v-if="!loadingCatalog && !datasetKeys.length" class="text-sm text-muted-foreground">
              The server catalog does not currently advertise any downloadable datasets.
            </p>
          </div>
    ...
    <Card v-if="task">
      <CardHeader class="flex-row items-start justify-between gap-4 space-y-0">
        <div class="min-w-0">
          ...
          <CardDescription class="mt-2 truncate font-mono">{{ task.id }}</CardDescription>
        </div>
        <Badge :variant="task.status === 'failed' ? 'destructive' : 'secondary'" class="capitalize">
          {{ task.status }}
        </Badge>
      </CardHeader>
      <CardContent class="space-y-4">
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="rounded-lg border bg-muted/30 p-4">
            <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Dataset key</p>
            <p class="mt-2 font-semibold">{{ task.job.dataset_key }}</p>
          </div>
          <div class="rounded-lg border bg-muted/30 p-4">
            <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Overwrite</p>
            <p class="mt-2 font-semibold">{{ task.job.overwrite ? 'Enabled' : 'Disabled' }}</p>
          </div>
        </div>
        ...
        <p v-if="!['succeeded', 'failed', 'cancelled'].includes(task.status)" class="text-sm text-muted-foreground">
    ...
    <Card v-else class="flex min-h-64 flex-col justify-center">
      <CardContent class="pt-6 text-center">
        ...
        <p class="mt-2 text-sm text-muted-foreground">Choose a configured dataset to start an asynchronous download.</p>
      </CardContent>
    </Card>
  </div>
  ...
  <Card class="mt-6 overflow-hidden">
    <CardHeader class="gap-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
      <div>
        ...
        <CardDescription class="mt-1">{{ total }} dataset{{ total === 1 ? '' : 's' }} in managed storage</CardDescription>
      </div>
      <Select v-model="status">
        <SelectTrigger class="w-full sm:w-44" aria-label="Filter datasets by status">
    ...
    <CardContent class="p-0">
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-if="loadingDatasets">
              <TableCell colspan="5" class="h-32 text-center text-muted-foreground">Loading managed datasets…</TableCell>
            </TableRow>
            <TableRow v-else-if="!datasets.length">
              <TableCell colspan="5" class="h-32 text-center text-muted-foreground">No datasets match this filter.</TableCell>
            </TableRow>
            <TableRow v-for="dataset in datasets" v-else :key="dataset.id">
              <TableCell>
                <p class="font-medium">{{ dataset.display_name }}</p>
                <p class="mt-1 font-mono text-xs text-muted-foreground">{{ dataset.dataset_key }}</p>
              </TableCell>
              <TableCell>
                <Badge :variant="dataset.status === 'failed' ? 'destructive' : 'secondary'" class="capitalize">
                  {{ dataset.status }}
                </Badge>
                <p v-if="dataset.error_code" class="mt-1 text-xs text-destructive">{{ dataset.error_code }}</p>
              </TableCell>
              <TableCell>{{ dataset.file_count ?? '—' }}</TableCell>
              <TableCell>{{ formatBytes(dataset.total_bytes) }}</TableCell>
              <TableCell class="whitespace-nowrap text-muted-foreground">{{ formatDate(dataset.updated_at) }}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </div>
      ...
      <div class="flex flex-col gap-3 border-t px-4 py-4 sm:flex-row sm:items-center sm:justify-between" aria-label="Dataset pagination">
        ...
        <div class="flex gap-2">
          ...
          <Button variant="outline" size="sm" :disabled="loadingDatasets || page >= pageCount" @click="setPage(page + 1)">Next</Button>
        </div>
      </div>
    </CardContent>
  </Card>
