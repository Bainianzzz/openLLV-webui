...
<template>
  <PageHeader
    eyebrow="Workspace"
    title="Train a model"
    description="Configure a training run from a managed dataset and follow it through completion."
  >
    <template #actions
      ><Badge variant="secondary">Training worker</Badge></template
    >
  </PageHeader>
  ...
  <form class="grid gap-6 xl:grid-cols-5" @submit.prevent="submit">
    <Card class="xl:col-span-3">
      <CardHeader>
        ...
        <CardDescription
          >Models and devices come from the server catalog. Only available
          managed datasets can be selected.</CardDescription
        >
      </CardHeader>
      <CardContent class="grid gap-5 sm:grid-cols-2">
        ...
        <div class="space-y-2 sm:col-span-2">
          ...
          <Select
            ...
            :disabled="loadingOptions || !datasets.length"
          >
            <SelectTrigger id="training-dataset" class="w-full"
              ><SelectValue placeholder="Select an available dataset"
            /></SelectTrigger>
            <SelectContent
              ><SelectItem
                ...
                :key="dataset.id"
                :value="dataset.id"
                >{{ dataset.display_name }}</SelectItem
              ></SelectContent
            >
          </Select>
          <p
            v-if="!loadingOptions && !datasets.length"
            class="text-sm text-muted-foreground"
          >
            No available datasets. Add or finish downloading one before
            training.
          </p>
        </div>
    ...
    <div class="space-y-6 xl:col-span-2">
      <Card>
        <CardHeader>
          ...
          <CardDescription
            >Optional experiment tracking. Credentials remain on the
        ...
        <CardContent class="space-y-5">
          ...
          <template v-if="form.useSwanLab">
            ...
            <div class="space-y-2">
              ...
              ><Input
                ...
                v-model="form.swanlabExperiment"
                placeholder="zero-dce-demo"
                required
              />
            </div>
          </template>
          ...
          >
            <AlertTitle>Training request failed</AlertTitle
          ...
          <div class="flex gap-3">
            <Button
              type="submit"
              class="flex-1"
              :disabled="loadingOptions || submitting || !datasets.length"
              :aria-busy="submitting"
              >{{ submitting ? "Submitting…" : "Start training" }}</Button
            >
            <Button
              v-if="error && !task"
              type="button"
    ...
    <CardContent class="space-y-5">
      ...
      <div v-if="task.status === 'succeeded'" class="grid gap-4 sm:grid-cols-3">
        ...
        <div class="rounded-lg border bg-muted/30 p-4">
          ...
          <p class="mt-2 text-2xl font-semibold">
            {{ task.job.best_val_loss ?? "Not reported" }}
          </p>
        </div>
        <div class="rounded-lg border bg-muted/30 p-4">
          <p
