"""Train openLLV models with SwanLab recording at both batch and epoch level.

openLLV's ``Trainer`` has no built-in SwanLab support, so it is subclassed
and hooked at two points:

- Batch level: ``BatchSwanLabTrainer`` overrides ``train_one_epoch``/``validate``
  to log the loss, learning rate, and gradient norm per batch.
- Epoch level: ``SwanLabTrainer`` overrides ``_save_history`` (called once per
  epoch by the base loop) to log the epoch summary.
...
class SwanLabTrainer(Trainer):
    ...
    Args:
        ...
        swan_api_key: SwanLab API key used to authenticate before
            ``swanlab.init``; ``None`` keeps the current login state.
    """
    ...
    def __init__(
        ...
        *args,
        ...
        swan_api_key: str | None = None,
        **kwargs,
    ) -> None:
        super().__init__(*args, **kwargs)
        if swan_api_key:
            swanlab.login(api_key=swan_api_key, relogin=True)
        swanlab.init(
